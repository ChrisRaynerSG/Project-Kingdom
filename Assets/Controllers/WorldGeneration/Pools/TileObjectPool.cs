...
public class TileObjectPool : MonoBehaviour
{
    ...
    public GameObject GetObject(Vector3 position, Transform parent)
    {
        ...
        if (canGrow)
        {
            GameObject obj = Instantiate(tilePrefab, position, Quaternion.identity, parent);
            return obj;
        }
    ...
    {
        obj.GetComponent<TileController>().ResetTile();
        obj.SetActive(false);
    ...
    private IEnumerator InitializePool()
    {
        ...
        {
            GameObject tile = Instantiate(tilePrefab, poolContainer);
